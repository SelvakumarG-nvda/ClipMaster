...
def safe_set_clipboard_text(text):
...
class ClipSpeakActions(object):
    ...
    def _is_clipspeak_enabled(self):
    ...
    def _classify_extension(self, ext):
        ext = ext.lower()
        if ext == '.py':
            return ".py file"
        elif ext == '.lnk':
    ...
    def _get_focused_file_type(self):
        try:
    ...
    def _get_clipboard_type_description(self):
        # 1. Native Explorer Files or Folders
        ...
        # 2. Text or File Paths copied as text
        try:
            text_val = safe_get_clipboard_text()
            if text_val:
                clean_val = text_val.strip('"').strip("'")
        ...
        focused = self._get_focused_file_type()
    ...
    def paste(self, gesture):
        ...
        core.callLater(100, _announce)

    def undo(self, gesture):
        gesture.send()
        if self._is_clipspeak_enabled():
            ui.message("Undo")

    def redo(self, gesture):
        gesture.send()
        if self._is_clipspeak_enabled():
            ui.message("Redo")

    def selectAll(self, gesture):
        gesture.send()
        if not self._is_clipspeak_enabled():
            return
    ...
    def save(self, gesture):
        ...
        if self._is_clipspeak_enabled():
            content_type = self._get_clipboard_type_description()
            ui.message(f"Saved {content_type}")

    def saveAll(self, gesture):
        gesture.send()
        if self._is_clipspeak_enabled():
            content_type = self._get_clipboard_type_description()
            ui.message(f"Save as {content_type}")

    def append(self, gesture):
        if not self._is_clipspeak_enabled():
            gesture.send()
            return
